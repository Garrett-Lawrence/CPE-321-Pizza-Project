...
public class ToppingsList {
    ...
    public double getToppingsPrice(){                                   // gets toppings price array list (toppingsList)
        ...
        for (int i = 0; i < toppingsList.size(); i++){
            sum += toppingsList.get(i).getToppingPrice();           // for every index in the array list, sums up the topping price by getting the price of each topping
        }
        toppingsPrice = sum;
        return sum;                                                 // returns topping price in variable sum
    }
}
